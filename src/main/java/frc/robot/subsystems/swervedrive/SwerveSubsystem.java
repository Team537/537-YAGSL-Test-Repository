...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      ...
      // files.
      // tr swerveDrive = new SwerveParser(directory).createSwerveDrive(6, 360,
  ...
  public Command targetRotation(Rotation2d rotation) {
    return run(
        ...
        .until(() -> Math.abs(getHeading().minus(rotation).getDegrees()) < 3); // TODO: Like, make the constant actually
  ...
  /**
   * Use PathPlanner Path finding to go to a point on the field. Drives at 2m/s to
   * allow for finer motion control
   *
   * @param pose Target {@link Pose2d} to go to.
   * @return PathFinding command
   */
  public Command driveToPoseFine(Pose2d pose, double errorToleranceMeters, double rotationErrorDEgrees) {

    // Create the constraints to use while pathfinding
    PathConstraints constraints = new PathConstraints(
        Units.feetToMeters(2), 4.0,
        swerveDrive.getMaximumChassisAngularVelocity(), Units.degreesToRadians(720));
    ...
    // End this command early when we’re “close enough”
    command = command.until(() -> {
      Pose2d current = getPose();
